...
public class PencilGame_Test {
	...
	static final int MAX = 4097;
	static long[] divisors = new long[MAX + 1];
	static int nDiv = 0;

	static final int MAXPRIME = 1000000;
	...
	static void parseDivisors(long x) {
		divisors[0] = 1;
		nDiv = 1;
	...
	static void parse(long x) {
		divisors[0] = 1;
		nDiv = 1;
		int limit = MAXPRIME;
		for (int i = 2; i < limit && i < x; i++) {
			if (x % i == 0) {
				divisors[nDiv++] = i;
			}
		}
	...
	static void test() {
		...
		long min = Long.MAX_VALUE;
		for (int t = 0; t < 150; t++) {
			long l = (long) (Math.random() * Math.pow(10, 12));
			parse(l);
			long s1 = sum();
			parseDivisors(l);
			long s2 = sum();
